...
class IssuingService:
    ...
    def create_vcc(self, db: Session, *, application_id: str, actor_id: str, actor_role: str, trace_id: str) -> VCCApplication:
        ...
        app.masked_card = result["masked_card"]
        app.status = transition_vcc(app.status, "ACTIVE")
        write_audit(db, actor_id=actor_id, actor_role=actor_role, action="CREATE_VCC", resource_type="VCCApplication", resource_id=app.id, trace_id=trace_id, outcome="ACTIVE")
        db.commit()
        return app
