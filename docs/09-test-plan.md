...
| PAY-09 | Concurrent business effect defense | HIGH | SUCCESS observation paths | Webhook×Webhook、Webhook×Query、Webhook×Reconciliation、底层 Observation | TOPUP ledger 一条；PostgreSQL row lock + DB unique constraint 为最终防线；SQLite 仅作为本地并发近似验证 | `test_concurrent_success_observations_have_one_credit_effect`, `test_concurrent_webhooks_have_one_credit_effect`, `test_concurrent_webhook_and_query_have_one_credit_effect`, `test_concurrent_webhook_and_reconciliation_have_one_credit_effect` | PASS |
...
| CON-03 | Webhook contract | HIGH | 官方 notification fixture | map legacy/current payment/refund | event type、IDs、amount/status 正确；current flat webhook 不解析为 Legacy envelope | `test_pingpong_query_refund_and_webhook_mapping`, `test_current_flat_webhook_contract_maps_without_legacy_envelope` | PASS |
...
| VCC-03 | Prompt injection / human confirmation | HIGH | unsafe message / approved application | 尝试绕过规则或未确认开卡 | Parser fail closed；结构化输出拒绝 extra 字段；Issuing Tool 拒绝无 human confirmation | `test_vcc_requires_human_approval_and_budget_gate`, `test_llm_parser_failure_fails_closed_to_clarification`, `test_vcc_request_rejects_prompt_injection_extra_fields`, `test_vcc_issuance_rejects_missing_human_confirmation` | PASS |
...
CI 使用 Mock-only pytest、ruff、mypy 和 coverage；真实 Sandbox acceptance 需另行准备凭证、签名密钥、产品权限和公网 HTTPS callback，当前为 Pending。
